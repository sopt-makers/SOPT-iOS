...
/// 모든 Coordinator가 따르는 Coordinator 프로토콜입니다.
public protocol TestCoordinator: AnyObject {
    /// start 메서드는 Coordinator가 담당하는 플로우를 시작하게 합니다. 대개의 경우 화면전환이 이뤄집니다.
    func start()
    ...
    func setDeallocallable(with object: TestCoordinatorFinishOutput)
    var deallocallable: TestCoordinatorFinishOutput? { get set }
}
